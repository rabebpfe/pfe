...
function charts(one) {
	...
	var n3 = [];
           ...
           for ( var key in obj) {
        	   ...
        	   for ( var key in obj) {
                   ...
                   crunchifyValue = crunchifyValue.substring(0,
                           crunchifyValue.length - 1);
                    n3 = crunchifyValue.split(",");
            
               }
        	   
        	     $("#mesure-count").text(n3[1]);
        		 $("#members-count").text(n3[2]);
        		 $("#watch-count").text(n3[3]);
        		 $("#patient-count").text(n3[4]);
        		 $("#risk-detected-count").text(n3[5]);
	...
	if($("#mesure").length)
	{
		...
		var plot = $.plot($("#mesure"),
			   [ { data: r1, label: "Fall"}, { data: r2, label: "Temperature" }, { data: r3, label: "Heartbeat" } ], {
				   series: {
					   lines: { show: true,
								lineWidth: 2,
							 },
		...
		$("#mesure").bind("plothover", function (event, pos, item) {
		...
		$("#mesure").bind("plotclick", function (event, pos, item) {
			if (item) {
	...
	/* ---------- Pie chart ---------- */
	var data = [
	{ label: "Admin",  data: adm},
	{ label: "Patient",  data: pat},
	{ label: "Supervisor",  data: sup}
